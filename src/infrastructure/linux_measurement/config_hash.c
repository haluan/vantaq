...
build_measurement_error_result(const char *path, vantaq_measurement_error_code_t error_code,
    ...
    vantaq_measurement_model_err_t model_err =
        vantaq_measurement_result_create_error("config_hash", path, error_code, out_result);
...
vantaq_config_hash_measure(const struct vantaq_runtime_config *config,
                           struct vantaq_measurement_result **out_result) {
    enum vantaq_config_hash_status status = VANTAQ_CONFIG_HASH_ERR_READ_FAILED;
    FILE *fp                              = NULL;
    long file_len                         = -1;
    ...
    *out_result = NULL;
    ...
    if (config_path == NULL || config_path[0] == '\0' || max_file_bytes == 0) {
        return VANTAQ_CONFIG_HASH_ERR_INVALID_ARG;
    }
    ...
    if (fp == NULL) {
        if (errno == ENOENT) {
            status = VANTAQ_CONFIG_HASH_ERR_SOURCE_NOT_FOUND;
            goto map_error;
        }
        status = VANTAQ_CONFIG_HASH_ERR_READ_FAILED;
        goto map_error;
    }
    ...
    if ((size_t)file_len > max_file_bytes) {
        status = VANTAQ_CONFIG_HASH_ERR_FILE_TOO_LARGE;
        goto map_error;
    }
    ...
    if (md_ctx == NULL) {
        status = VANTAQ_CONFIG_HASH_ERR_HASH_FAILED;
        goto map_error;
    }
    ...
    if (vantaq_measurement_result_create_success("config_hash", value, config_path, out_result) !=
        VANTAQ_MEASUREMENT_MODEL_OK) {
        status = VANTAQ_CONFIG_HASH_ERR_MODEL_FAILED;
        goto cleanup;
    }

    status = VANTAQ_CONFIG_HASH_OK;
    goto cleanup;

map_error:
    if (status == VANTAQ_CONFIG_HASH_ERR_SOURCE_NOT_FOUND) {
        enum vantaq_config_hash_status map_status =
            build_measurement_error_result(config_path, MEASUREMENT_SOURCE_NOT_FOUND, out_result);
        if (map_status != VANTAQ_CONFIG_HASH_OK) {
            status = map_status;
        }
    } else if (status == VANTAQ_CONFIG_HASH_ERR_HASH_FAILED) {
        enum vantaq_config_hash_status map_status =
            build_measurement_error_result(config_path, MEASUREMENT_HASH_FAILED, out_result);
        if (map_status != VANTAQ_CONFIG_HASH_OK) {
            status = map_status;
        }
    } else {
        enum vantaq_config_hash_status map_status =
            build_measurement_error_result(config_path, MEASUREMENT_READ_FAILED, out_result);
        if (map_status != VANTAQ_CONFIG_HASH_OK) {
            status = map_status;
        }
    }
