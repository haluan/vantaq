...
vantaq_evidence_err_t vantaq_evidence_serialize_canonical(const struct vantaq_evidence *evidence,
    ...
    const char *format = "challenge_id:%s|claims:%s|device_id:%s|evidence_id:%s|issued_at_unix:%"
                         "lld|nonce:%s|purpose:%s|signature_alg:%s|verifier_id:%s";

    // 1. Calculate required length
    int len = snprintf(NULL, 0, format, ch_id, claims, dev_id, ev_id, (long long)issued_at, nonce,
                       purpose, sig_alg, ver_id);
    if (len < 0) {
    ...
    // 3. Populate buffer
    int written = snprintf(buffer, len + 1, format, ch_id, claims, dev_id, ev_id,
                           (long long)issued_at, nonce, purpose, sig_alg, ver_id);
    if (written != len) {
        free(buffer);
        return VANTAQ_EVIDENCE_ERR_INVALID_ARG;
    }
