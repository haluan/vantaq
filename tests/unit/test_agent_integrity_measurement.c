...
struct AgentIntegrityMeasurementTestSuite {
...
static int suite_setup(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s =
        calloc(1, sizeof(struct AgentIntegrityMeasurementTestSuite));
    if (!s) {
        return -1;
    }

    snprintf(s->agent_binary_path, sizeof(s->agent_binary_path), "/tmp/vantaq_agent_bin_%d.bin",
             getpid());
    *state = s;
    return 0;
}
...
static void fill_measurement_config(struct vantaq_runtime_config *config, const char *agent_path,
...
static void test_agent_integrity_success(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    const unsigned char content[]            = "vantaqd-binary-v1";

    assert_int_equal(write_file(s->agent_binary_path, content, sizeof(content) - 1), 0);
    ...
    s_assert_int_equal(s, status, VANTAQ_AGENT_INTEGRITY_OK);
    ...
    s_assert_string_equal(s, vantaq_measurement_result_get_claim_name(result), "agent_integrity");
    s_assert_string_equal(s, vantaq_measurement_result_get_source_path(result),
                          s->agent_binary_path);
...
static void test_agent_integrity_missing_source(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    ...
    s_assert_int_equal(s, status, VANTAQ_AGENT_INTEGRITY_ERR_SOURCE_NOT_FOUND);
    s_assert_non_null(s, result);
    s_assert_int_equal(s, vantaq_measurement_result_get_status(result),
...
static void test_agent_integrity_deterministic_and_changes(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    ...
    const unsigned char content_a[]           = "agent-a";
    const unsigned char content_b[]           = "agent-b";

    fill_measurement_config(&config, s->agent_binary_path, 1024);

    assert_int_equal(write_file(s->agent_binary_path, content_a, sizeof(content_a) - 1), 0);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, &result1),
                       VANTAQ_AGENT_INTEGRITY_OK);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, &result2),
                       VANTAQ_AGENT_INTEGRITY_OK);
    s_assert_string_equal(s, vantaq_measurement_result_get_value(result1),
                          vantaq_measurement_result_get_value(result2));

    assert_int_equal(write_file(s->agent_binary_path, content_b, sizeof(content_b) - 1), 0);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, &result3),
                       VANTAQ_AGENT_INTEGRITY_OK);
...
static void test_agent_integrity_invalid_args(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;

    s_assert_int_equal(s, vantaq_agent_integrity_measure(NULL, &result),
                       VANTAQ_AGENT_INTEGRITY_ERR_INVALID_ARG);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, NULL),
                       VANTAQ_AGENT_INTEGRITY_ERR_INVALID_ARG);

    fill_measurement_config(&config, "", 1024);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, &result),
                       VANTAQ_AGENT_INTEGRITY_ERR_INVALID_ARG);
    s_assert_null(s, result);

    fill_measurement_config(&config, s->agent_binary_path, 0);
    s_assert_int_equal(s, vantaq_agent_integrity_measure(&config, &result),
                       VANTAQ_AGENT_INTEGRITY_ERR_INVALID_ARG);
    s_assert_null(s, result);
}

static void test_agent_integrity_file_too_large(void **state) {
    struct AgentIntegrityMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    ...
    assert_int_equal(write_file(s->agent_binary_path, content, sizeof(content) - 1), 0);
    fill_measurement_config(&config, s->agent_binary_path, 4);

    enum vantaq_agent_integrity_status status = vantaq_agent_integrity_measure(&config, &result);

    s_assert_int_equal(s, status, VANTAQ_AGENT_INTEGRITY_ERR_FILE_TOO_LARGE);
    s_assert_non_null(s, result);
    s_assert_int_equal(s, vantaq_measurement_result_get_status(result),
                       VANTAQ_MEASUREMENT_STATUS_ERROR);
    s_assert_int_equal(s, vantaq_measurement_result_get_error_code(result),
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_agent_integrity_missing_source, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_agent_integrity_deterministic_and_changes, suite_setup,
                                        suite_teardown),
    ...
    return cmocka_run_group_tests_name("agent_integrity_measurement_suite", tests, NULL, NULL);
}
