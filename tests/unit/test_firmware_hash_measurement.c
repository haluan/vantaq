...
static int suite_setup(void **state) {
    struct FirmwareHashMeasurementTestSuite *s =
        calloc(1, sizeof(struct FirmwareHashMeasurementTestSuite));
    if (!s) {
        return -1;
    }

    snprintf(s->firmware_path, sizeof(s->firmware_path), "/tmp/vantaq_firmware_%d.bin", getpid());
    *state = s;
    return 0;
}
...
static void fill_measurement_config(struct vantaq_runtime_config *config, const char *firmware_path,
                                    size_t max_file_bytes) {
    VANTAQ_ZERO_STRUCT(*config);
    config->cbSize = sizeof(*config);
    strncpy(config->measurement_firmware_path, firmware_path,
    ...
    strncpy(config->measurement_agent_binary_path, "/etc/hosts",
...
static void test_firmware_hash_success(void **state) {
    struct FirmwareHashMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    const unsigned char content[]            = "firmware-v1";

    assert_int_equal(write_file(s->firmware_path, content, sizeof(content) - 1), 0);
    ...
    s_assert_int_equal(s, status, VANTAQ_FIRMWARE_HASH_OK);
    ...
    s_assert_string_equal(s, vantaq_measurement_result_get_claim_name(result), "firmware_hash");
    s_assert_string_equal(s, vantaq_measurement_result_get_source_path(result), s->firmware_path);
...
static void test_firmware_hash_missing_source(void **state) {
    struct FirmwareHashMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    ...
    s_assert_int_equal(s, status, VANTAQ_FIRMWARE_HASH_ERR_SOURCE_NOT_FOUND);
    s_assert_non_null(s, result);
    s_assert_int_equal(s, vantaq_measurement_result_get_status(result),
...
static void test_firmware_hash_file_too_large(void **state) {
    struct FirmwareHashMeasurementTestSuite *s = *state;
    struct vantaq_runtime_config config;
    struct vantaq_measurement_result *result = NULL;
    const unsigned char content[]            = "firmware-v1-oversized";

    assert_int_equal(write_file(s->firmware_path, content, sizeof(content) - 1), 0);
    fill_measurement_config(&config, s->firmware_path, 4);

    enum vantaq_firmware_hash_status status = vantaq_firmware_hash_measure(&config, &result);

    s_assert_int_equal(s, status, VANTAQ_FIRMWARE_HASH_ERR_FILE_TOO_LARGE);
    s_assert_non_null(s, result);
    s_assert_int_equal(s, vantaq_measurement_result_get_status(result),
                       VANTAQ_MEASUREMENT_STATUS_ERROR);
    s_assert_int_equal(s, vantaq_measurement_result_get_error_code(result),
...
static void test_firmware_hash_deterministic_and_changes(void **state) {
    ...
    const unsigned char content_a[]           = "firmware-a";
    const unsigned char content_b[]           = "firmware-b";

    fill_measurement_config(&config, s->firmware_path, 1024);

    assert_int_equal(write_file(s->firmware_path, content_a, sizeof(content_a) - 1), 0);
    s_assert_int_equal(s, vantaq_firmware_hash_measure(&config, &result1), VANTAQ_FIRMWARE_HASH_OK);
    s_assert_int_equal(s, vantaq_firmware_hash_measure(&config, &result2), VANTAQ_FIRMWARE_HASH_OK);
    s_assert_string_equal(s, vantaq_measurement_result_get_value(result1),
                          vantaq_measurement_result_get_value(result2));

    assert_int_equal(write_file(s->firmware_path, content_b, sizeof(content_b) - 1), 0);
    s_assert_int_equal(s, vantaq_firmware_hash_measure(&config, &result3), VANTAQ_FIRMWARE_HASH_OK);
...
static void test_firmware_hash_invalid_args(void **state) {
    ...
    s_assert_int_equal(s, vantaq_firmware_hash_measure(NULL, &result),
                       VANTAQ_FIRMWARE_HASH_ERR_INVALID_ARG);
    s_assert_int_equal(s, vantaq_firmware_hash_measure(&config, NULL),
                       VANTAQ_FIRMWARE_HASH_ERR_INVALID_ARG);
    ...
    fill_measurement_config(&config, s->firmware_path, 0);
    s_assert_int_equal(s, vantaq_firmware_hash_measure(&config, &result),
                       VANTAQ_FIRMWARE_HASH_ERR_INVALID_ARG);
    s_assert_null(s, result);
}
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_firmware_hash_success, suite_setup, suite_teardown),
        cmocka_unit_test_setup_teardown(test_firmware_hash_missing_source, suite_setup,
                                        suite_teardown),
    ...
    return cmocka_run_group_tests_name("firmware_hash_measurement_suite", tests, NULL, NULL);
}
