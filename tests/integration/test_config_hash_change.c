...
static int curl_mtls_post_status_body(int port, const char *path, const char *payload,
    ...
    snprintf(body_file, sizeof(body_file), "/tmp/vantaq_cfg_change_body_%d.tmp", getpid());
...
static int suite_setup(void **state) {
    struct ConfigHashChangeSuite *s = malloc(sizeof(*s));
    ...
    snprintf(s->firmware_path, sizeof(s->firmware_path), "/tmp/vantaq_firmware_cfg_change_%d.bin",
             getpid());
    snprintf(s->security_config_path, sizeof(s->security_config_path),
             "/tmp/vantaq_security_cfg_change_%d.conf", getpid());
    if (write_file(s->firmware_path, "firmware-content-seed") != 0 ||
        write_file(s->security_config_path, "policy=seed\n") != 0) {
    ...
    if (vantaq_test_server_start(&opts, &s->server, setup_err, sizeof(setup_err)) != 0) {
        if (setup_err[0] != '\0') {
            print_error("test_config_hash_change setup failed: %s\n", setup_err);
        }
...
static int suite_teardown(void **state) {
    struct ConfigHashChangeSuite *s = *state;
    if (s != NULL) {
...
static void test_config_hash_changes_after_security_config_file_update(void **state) {
    struct ConfigHashChangeSuite *s = *state;
    ...
    s_assert_int_equal(s, write_file(s->security_config_path, "policy=v1\n"), 0);
    s_assert_int_equal(
        s,
    ...
    s_assert_int_equal(s, extract_config_hash(body_a, hash_a, sizeof(hash_a)), 0);
    s_assert_true(s, strncmp(hash_a, "sha256:", 7) == 0);

    snprintf(evidence_path_a, sizeof(evidence_path_a), "/tmp/vantaq_cfg_change_evidence_a_%d.json",
             getpid());
    s_assert_int_equal(s, verify_evidence_signature(body_a, evidence_path_a), 0);

    s_assert_int_equal(s, write_file(s->security_config_path, "policy=v2\nallow=true\n"), 0);
    s_assert_int_equal(
        s,
        request_challenge(s->server.port, challenge_id, sizeof(challenge_id), nonce, sizeof(nonce)),
        0);
    ...
    s_assert_int_equal(s,
                       ...
                       0);
    s_assert_int_equal(s, status, 200);
    s_assert_int_equal(s, extract_config_hash(body_b, hash_b, sizeof(hash_b)), 0);
    s_assert_true(s, strncmp(hash_b, "sha256:", 7) == 0);

    snprintf(evidence_path_b, sizeof(evidence_path_b), "/tmp/vantaq_cfg_change_evidence_b_%d.json",
             getpid());
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_config_hash_changes_after_security_config_file_update,
                                        suite_setup, suite_teardown),
    };

    return cmocka_run_group_tests_name("integration_config_hash_change", tests, NULL, NULL);
}
