...
struct EvidenceAgentIntegritySuite {
...
static int curl_mtls_post_status_body(int port, const char *path, const char *payload,
    ...
    snprintf(body_file, sizeof(body_file), "/tmp/vantaq_agent_claim_body_%d.tmp", getpid());
...
static int suite_setup(void **state) {
    struct EvidenceAgentIntegritySuite *s = malloc(sizeof(*s));
    ...
    snprintf(s->firmware_path, sizeof(s->firmware_path), "/tmp/vantaq_firmware_agent_claim_%d.bin",
             getpid());
    snprintf(s->security_config_path, sizeof(s->security_config_path),
             "/tmp/vantaq_security_agent_claim_%d.conf", getpid());
    snprintf(s->agent_binary_path, sizeof(s->agent_binary_path),
             "/tmp/vantaq_agent_binary_claim_%d.bin", getpid());
    ...
    if (vantaq_test_server_start(&opts, &s->server, setup_err, sizeof(setup_err)) != 0) {
        if (setup_err[0] != '\0') {
            print_error("test_evidence_agent_integrity_claim setup failed: %s\n", setup_err);
        }
...
static int suite_teardown(void **state) {
    struct EvidenceAgentIntegritySuite *s = *state;
    if (s != NULL) {
...
static void test_agent_integrity_requested_included_and_signature_valid(void **state) {
    struct EvidenceAgentIntegritySuite *s = *state;
    ...
    snprintf(req, sizeof(req),
             "{\"challenge_id\":\"%s\",\"nonce\":\"%s\",\"claims\":[\"agent_integrity\"]}",
             challenge_id, nonce);
    ...
    rc = system("./bin/verify_evidence /tmp/vantaq_agent_claim_evidence.json "
                "config/certs/device-server.crt > /dev/null 2>&1");
    s_assert_int_equal(s, WEXITSTATUS(rc), 0);
    unlink("/tmp/vantaq_agent_claim_evidence.json");
}

static void test_agent_integrity_not_requested_omitted(void **state) {
    struct EvidenceAgentIntegritySuite *s = *state;
    char challenge_id[128];
    char nonce[128];
    char req[768];
...
static void test_agent_integrity_missing_source_returns_404(void **state) {
    struct EvidenceAgentIntegritySuite *s = *state;
    char challenge_id[128];
    char nonce[128];
    char req[768];
...
static void test_claim_not_allowed_returns_403(void **state) {
    ...
    snprintf(firmware_path, sizeof(firmware_path), "/tmp/vantaq_firmware_agent_claim_deny_%d.bin",
             getpid());
    snprintf(security_config_path, sizeof(security_config_path),
             "/tmp/vantaq_security_agent_claim_deny_%d.conf", getpid());
    snprintf(agent_binary_path, sizeof(agent_binary_path),
             "/tmp/vantaq_agent_binary_claim_deny_%d.bin", getpid());
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_agent_integrity_requested_included_and_signature_valid,
                                        suite_setup, suite_teardown),
        cmocka_unit_test_setup_teardown(test_agent_integrity_not_requested_omitted, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_agent_integrity_missing_source_returns_404,
                                        suite_setup, suite_teardown),
        cmocka_unit_test_setup_teardown(test_claim_not_allowed_returns_403, suite_setup,
                                        suite_teardown),
    };

    return cmocka_run_group_tests_name("integration_evidence_agent_integrity_claim", tests, NULL,
                                       NULL);
}
