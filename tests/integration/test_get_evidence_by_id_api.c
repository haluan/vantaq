...
struct GetEvidenceByIdApiSuite {
    char temp_dir[256];
};
...
static int make_temp_dir(char *out, size_t out_size) {
    char templ[] = "/tmp/vantaq_t09_XXXXXX";
    char *dir;
...
static int start_server_with_ring(struct vantaq_test_server_handle *server, const char *ring_path,
    ...
    if (vantaq_test_server_start(&opts, server, err, err_len) != 0) {
        if (err != NULL && err[0] != '\0') {
            print_error("test_get_evidence_by_id_api setup failed: %s\n", err);
        }
        return -1;
    }
...
static int suite_setup(void **state) {
    struct GetEvidenceByIdApiSuite *s = calloc(1, sizeof(*s));
...
static int suite_teardown(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
...
static void test_existing_visible_evidence_id_returns_200(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    ...
    s_assert_int_equal(
        s, extract_json_string_field(post_body, "evidence_id", evidence_id, sizeof(evidence_id)),
        0);

    (void)snprintf(get_path, sizeof(get_path), "/v1/attestation/evidence/%s", evidence_id);
    s_assert_int_equal(
        s,
        curl_mtls_request(server.port, "GET", get_path, NULL, &status, get_body, sizeof(get_body)),
        0);
    s_assert_int_equal(s, status, 200);
    s_assert_int_equal(s,
                       ...
                       0);
    s_assert_int_equal(s, strcmp(get_evidence_id, evidence_id), 0);

    vantaq_test_server_stop(&server);
    (void)unlink(ring_path);
}

static void test_unknown_evidence_id_returns_404(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    struct vantaq_test_server_handle server;
    char ring_path[512];
    char setup_err[512];
    ...
    s_assert_int_equal(
        s, make_temp_file_path(s->temp_dir, "ring_unknown", ".ring", ring_path, sizeof(ring_path)),
        0);
    ...
    s_assert_int_equal(s,
                       curl_mtls_request(server.port, "GET",
                                         "/v1/attestation/evidence/ev-does-not-exist", NULL,
...
static void test_invalid_evidence_id_path_returns_400(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    struct vantaq_test_server_handle server;
    char ring_path[512];
    char setup_err[512];
    ...
    s_assert_int_equal(
        s, make_temp_file_path(s->temp_dir, "ring_invalid", ".ring", ring_path, sizeof(ring_path)),
        0);
    ...
    s_assert_int_equal(s,
                       curl_mtls_request(server.port, "GET", "/v1/attestation/evidence/", NULL,
                                         &status, body, sizeof(body)),
                       0);
    s_assert_int_equal(s, status, 400);

    s_assert_int_equal(s,
                       curl_mtls_request(server.port, "GET", "/v1/attestation/evidence/ev-x/extra",
                                         NULL, &status, body, sizeof(body)),
                       0);
    s_assert_int_equal(s, status, 400);

    s_assert_int_equal(s,
                       curl_mtls_request(server.port, "GET", "/v1/attestation/evidence/%2e%2e",
                                         NULL, &status, body, sizeof(body)),
                       0);
    s_assert_int_equal(s, status, 400);
...
static void test_cross_verifier_evidence_returns_404(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    struct vantaq_test_server_handle server;
    char ring_path[512];
    char setup_err[512];
    ...
    s_assert_int_equal(
        s, make_temp_file_path(s->temp_dir, "ring_cross", ".ring", ring_path, sizeof(ring_path)),
        0);

    s_assert_int_equal(
        s,
        append_record_to_ring(
            ring_path, "ev-owned-by-other", "other-verifier-01",
            "{\"evidence_id\":\"ev-owned-by-other\",\"verifier_id\":\"other-verifier-01\"}",
            1770300101),
        0);
    ...
    s_assert_int_equal(s,
                       curl_mtls_request(server.port, "GET",
                                         "/v1/attestation/evidence/ev-owned-by-other", NULL,
                                         &status, body, sizeof(body)),
                       0);
    s_assert_int_equal(s, status, 404);
...
static void test_signature_integrity_unchanged_for_get_by_id(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    struct vantaq_test_server_handle server;
    char ring_path[512];
    char setup_err[512];
    ...
    s_assert_int_equal(s,
                       make_temp_file_path(s->temp_dir, "by_id_payload", ".json",
...
static void test_latest_route_not_treated_as_by_id(void **state) {
    struct GetEvidenceByIdApiSuite *s = *state;
    struct vantaq_test_server_handle server;
    char ring_path[512];
    char setup_err[512];
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_existing_visible_evidence_id_returns_200, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_unknown_evidence_id_returns_404, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_invalid_evidence_id_path_returns_400, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_cross_verifier_evidence_returns_404, suite_setup,
                                        suite_teardown),
        cmocka_unit_test_setup_teardown(test_signature_integrity_unchanged_for_get_by_id,
                                        suite_setup, suite_teardown),
        cmocka_unit_test_setup_teardown(test_latest_route_not_treated_as_by_id, suite_setup,
                                        suite_teardown),
    };

    return cmocka_run_group_tests_name("integration_get_evidence_by_id_api", tests, NULL, NULL);
}
